...
protocol SuggestionOverlayControlling: AnyObject {
    ...
    /// the width the consumed prefix occupied in the ghost's own render font (whether Cotabby typed
    /// that prefix on accept or the user typed it through). The slide reads the held overlay state,
    /// never a fresh AX caret, so it cannot jitter against AX noise. Returns `false` when the
    /// controller cannot safely slide (hidden, mirror mode, RTL, multi-line, or nothing rendered to
    /// measure against); callers then fall back to a caret-anchored present.
